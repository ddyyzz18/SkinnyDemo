...
void main()
{
    ...
    mat4 finalnormal = transpose(inverse(finalmodel)); // animation transform for normal vector

    vec3 PosW = vec3(finalmodel*vec4(pos,1.0));
    vs_out.TexCoords = tex;

    vec3 T = vec3(normalize(finalnormal*vec4(tangent,1.0)));
    vec3 N = vec3(normalize(finalnormal*vec4(norm,1.0)));
